## staghmc_sssh.nim (symanzik gaction,  stoutsmear faction) with Hasenbusch masses.

import qex, gauge, gauge/stoutsmear, physics/qcdTypes, physics/stagSolve
import mdevolve
...
let
  ...
  gc = GaugeActionCoeffs(plaq: (beta*5/3) , rect: beta*(-1/12))
  vol = lo.physVol
...
var
  info: PerfInfo
  coef = StoutCoefs(n:2, rho:0.15, layout: lo)
echo "smear = ",coef
